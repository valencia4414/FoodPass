...
<body class="bg-fp-bg text-fp-darkgreen antialiased h-screen overflow-hidden flex" x-data="carritoApp()">

    <!-- Sidebar Izquierdo Fijo -->
    <aside
        class="w-56 bg-fp-darkgreen h-screen flex flex-col justify-between fixed left-0 top-0 z-20 text-white shrink-0">
        <div>
            <div class="px-6 py-8">
                <h1 class="text-2xl font-bold tracking-tight text-fp-orange">FoodPass</h1>
                <p class="text-[10px] text-gray-400 font-medium uppercase tracking-widest mt-1">Gestión Alimentaria SENA
                </p>
            </div>

            <nav class="px-3 flex flex-col gap-1">
                <a href="{{ route('dashboard') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-300 hover:bg-white/10 hover:text-white transition-colors">
                    <span class="material-symbols-outlined text-[20px]">home</span>
                    <span class="text-sm font-medium">Inicio</span>
                </a>
                <a href="{{ route('menu-digital') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-fp-orange text-white shadow-md shadow-fp-orange/20 transition-colors">
        ...
        <div class="p-4 mb-4 mx-3 rounded-xl bg-black/30 border border-white/10 flex items-center gap-3">
            <div
                class="w-10 h-10 rounded-full bg-fp-orange flex items-center justify-center font-bold text-white shrink-0 shadow-inner">
    ...
    <div class="ml-56 flex-1 flex flex-col h-screen w-full relative">
        ...
        <header
            class="h-16 bg-white/90 backdrop-blur-md border-b border-gray-200/60 flex items-center justify-between px-8 sticky top-0 z-10 w-full">
            ...
            <div class="relative w-96">
                ...
                <input type="text" x-model="busqueda" placeholder="Buscar platillos o ingredientes..."
            ...
            <div class="flex items-center gap-5">
                ...
                <div class="flex items-center gap-2 border-l border-gray-200 pl-5">
                    <span
                        class="text-sm font-medium text-fp-darkgreen">{{ auth()->user()->name ?? 'Usuario SENA' }}</span>
                    <div
                        class="w-8 h-8 rounded-full bg-fp-green text-white flex items-center justify-center text-xs font-bold">
                        {{ substr(auth()->user()->name ?? 'U', 0, 1) }}
                    </div>
                </div>
            </div>
        ...
        <main class="flex-1 overflow-y-auto p-8 pb-20">
            ...
            <section class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                <div class="lg:col-span-2 relative rounded-3xl overflow-hidden h-64 shadow-md group">
                    @if($platilloHero)
                        ...
                        <div class="relative h-full flex flex-col justify-center p-8 lg:w-2/3">
                            <div
                                class="inline-block px-3 py-1 bg-fp-orange text-white text-[10px] font-bold uppercase tracking-wider rounded-md mb-3 self-start">
                                Plato Recomendado del Día</div>
                            ...
                            <button
                                @click="agregarAlCarrito({ id: {{ $platilloHero->id }}, nombre: '{{ addslashes($platilloHero->nombre) }}', precio: {{ $platilloHero->precio }} })"
                ...
                <div class="flex flex-col gap-4 h-64">
                    <div
                        class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex-1 flex flex-col justify-center">
                        <div class="flex items-center gap-3 mb-1">
                            <div
                                class="w-10 h-10 rounded-full bg-fp-orange/10 flex items-center justify-center text-fp-orange">
                                <span class="material-symbols-outlined">stars</span>
                            </div>
                            <h3 class="font-bold text-base text-fp-darkgreen">Puntos FoodPass</h3>
                        </div>
                        <p class="text-gray-500 text-xs mb-2">Tienes <span class="font-bold text-fp-green">1,250</span>
                            puntos acumulados.</p>
                        <a href="{{ route('canje') }}"
                            class="text-fp-orange text-xs font-semibold hover:underline inline-flex items-center gap-1">Ver
                            catálogo de canje <span
                                class="material-symbols-outlined text-[14px]">arrow_forward</span></a>
                    </div>
                    <div
                        class="bg-fp-card-light rounded-3xl p-6 shadow-sm flex-1 flex flex-col justify-center border border-[#d2ebaf]">
            ...
            <section>
                ...
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
                    ...
                    <div
                        class="flex items-center gap-2 p-1.5 bg-white rounded-xl shadow-sm border border-gray-200 overflow-x-auto">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    @forelse($platillos as $platillo)
                        <div x-show="cumpleFiltro('{{ $platillo->categoria }}', '{{ strtolower($platillo->nombre) }}', '{{ strtolower($platillo->descripcion) }}')"
                            ...
                            <div class="relative w-full h-40 rounded-2xl overflow-hidden mb-3 cursor-pointer"
                                ...
                                <!-- Badge de Categoría -->
                                <span
                                    class="absolute top-3 left-3 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase bg-fp-darkgreen text-white">
                                ...
                                @if(!$platillo->disponible)
                                    <div class="absolute inset-0 bg-black/50 flex items-center justify-center">
                                        <span
                                            class="px-3 py-1 bg-red-600 text-white text-xs font-bold uppercase tracking-wider rounded-lg shadow-md">AGOTADO</span>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="flex items-center justify-between pt-2 border-t border-gray-50 mt-auto">
                                <span
                                    class="font-bold text-lg text-fp-orange">${{ number_format($platillo->precio, 0, ',', '.') }}</span>

                                <!-- Tarea 13: Botón Seleccionar directo sin advertencias de sintaxis -->
                                @if($platillo->disponible)
                                    <button
                                        @click="agregarAlCarrito({ id: {{ $platillo->id }}, nombre: '{{ addslashes($platillo->nombre) }}', precio: {{ $platillo->precio }} })"
                    ...
                    @empty
                        <div class="col-span-full py-12 text-center text-gray-400">No hay platillos registrados en el
                            sistema.</div>
                    @endforelse
    ...
    <div x-show="modalDetalle" x-cloak
        class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" x-transition>
        <div class="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl relative"
            ...
            <button @click="modalDetalle = false"
                class="absolute top-4 right-4 bg-white/80 rounded-full p-1.5 text-gray-600 hover:text-black z-10">
                ...
                <div>
                    ...
                    <div class="p-6">
                        <div class="flex justify-between items-start mb-2">
                            ...
                            <span class="text-xl font-bold text-fp-orange"
                                x-text="'$' + Number(platilloSeleccionado.precio).toLocaleString('es-CO')"></span>
                        </div>
                        ...
                        <div class="mb-6 bg-fp-bg p-4 rounded-xl border border-gray-100">
                            <h4 class="text-xs font-bold uppercase tracking-wider text-fp-darkgreen mb-1">Ingredientes:
                            </h4>
                            <p class="text-xs text-gray-500"
                                x-text="platilloSeleccionado.ingredientes || 'Ingredientes frescos seleccionados del día.'">
                            </p>
                        </div>
    ...
    <div x-show="abrirCarrito" x-cloak class="fixed inset-0 z-50 overflow-hidden bg-black/50 backdrop-blur-sm"
        x-transition>
        <div class="absolute inset-y-0 right-0 max-w-full flex pl-10">
            <div class="w-screen max-w-md bg-white shadow-xl flex flex-col justify-between"
                @click.away="abrirCarrito = false">
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-4">
                    <template x-if="carrito.length === 0">
                        <div class="h-full flex flex-col items-center justify-center text-gray-400 py-12">
                            <span
                                class="material-symbols-outlined text-5xl mb-2 text-gray-300">remove_shopping_cart</span>
                    ...
                    <template x-for="(item, index) in carrito" :key="item.id">
                        <div class="flex items-center justify-between p-3 bg-fp-bg rounded-2xl border border-gray-100">
                            <div class="flex-1 pr-3">
                                ...
                                <span class="text-xs font-semibold text-fp-orange"
                                    x-text="'$' + Number(item.precio * item.cantidad).toLocaleString('es-CO')"></span>
                            </div>
                            ...
                            <div class="flex items-center gap-2 bg-white px-2 py-1 rounded-lg border border-gray-200">
                                <button @click="disminuirCantidad(index)"
                                    class="text-gray-500 hover:text-red-500 font-bold px-1 text-sm">-</button>
                                <span class="text-xs font-bold w-4 text-center" x-text="item.cantidad"></span>
                                <button @click="aumentarCantidad(index)"
                                    class="text-gray-500 hover:text-fp-green font-bold px-1 text-sm">+</button>
                            </div>
                        </div>
                    </template>
                </div>
                ...
                <div class="p-6 border-t border-gray-100 bg-white space-y-4">
                    <div class="flex justify-between items-center text-sm font-medium text-gray-600">
                        <span>Subtotal:</span>
                        <span class="font-bold text-fp-darkgreen"
                            x-text="'$' + totalPrecio.toLocaleString('es-CO')"></span>
                    </div>
                    <div
                        class="flex justify-between items-center text-lg font-bold text-fp-darkgreen border-t border-gray-100 pt-2">
                        <span>Total Pedido:</span>
                        <span class="text-fp-orange" x-text="'$' + totalPrecio.toLocaleString('es-CO')"></span>
                    </div>

                    <button @click="abrirConfirmacion = true; abrirCarrito = false;" :disabled="carrito.length === 0"
    ...
    <div x-show="abrirConfirmacion" x-cloak
        class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" x-transition>
        <div class="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl relative"
            @click.away="abrirConfirmacion = false">
            ...
            <div class="max-h-40 overflow-y-auto mb-4 border-y border-gray-100 py-2 space-y-2">
                <template x-for="item in carrito" :key="item.id">
                    <div class="flex justify-between text-xs">
                        ...
                        <span x-text="'$' + Number(item.precio * item.cantidad).toLocaleString('es-CO')"
            ...
            <div class="mb-6">
                <label class="block text-xs font-bold uppercase tracking-wider text-fp-darkgreen mb-2">Método de
                    Pago:</label>
        ...
        function carritoApp() {
            return {
                ...
                agregarAlCarrito(platillo) {
                    // Sincronizar el producto con el carrito de sesión de Laravel
                    fetch("/cart/add", {
                        method: 'POST',
                        headers: {
                        ...
                        .then(res => {
                            ...
                            // Actualizar también el carrito visual
                        ...
                        .catch(err => {
                            ...
                            // Fallback local si falla la petición al servidor
                            const existente = this.carrito.find(item => item.id === platillo.id);
